...
<script type="text/javascript">
	function cha() {
		...
		window.location.href = "downloadList.do?pageNum="
				+ pageNum;
	}
</script>
		...
		<div class="sidebar fl">
			...
			<div class="search_box">
				<form action="indexSearch.do" class="search_bar" method="post">
					<input type="text" class="search_bar_text" value="" /> <input
						type="button" class="search_bar_btn" value="站内搜索" />
				</form>
			</div>
			-->
		</div>
					...
					<div id="col_content1" class="column_content fl">
						<ul>
							...
							<c:forEach items="${accessoryList}" var="accessory">
								...
								<li class="clo"><span class="fr ar">${accessory.upLoadTime}</span><a
									href="downLoad.do?id=${accessory.id}">${accessory.fileName}</a></li>
							</c:forEach>

						</ul>
						<div id="page">

							<font size="2">共显示 ${page.totalCount} 个文件</font> <font size="2">每页
								${page.pageSize} 个</font> <font size="2">共 ${page.totalPageCount} 页</font>
							<font size="2">第 ${page.getPageNum()} 页</font>
							<c:choose>
								<c:when test="${page.pageNum - 1 > 0}">
									<a
										class="page" href="/LTXProject/downloadList.do?pageNum=${page.pageNum - 1}">[上一页]</a>
								</c:when>
							...
							<c:choose>
								<c:when test="${page.pageNum != page.totalPageCount}">
									<a
										class="page" href="/LTXProject/downloadList.do?pageNum=${page.pageNum + 1}">[下一页]</a>
								</c:when>
