...
<script type="text/javascript">
	function cha() {
		...
		window.location.href = "indexSearch.do?pageNum="
				+ pageNum+"&articleTitle=${sessionScope.articleTitle}";
	}
</script>
	...
	<div class="header">
		...
		<div id="search_box">
			<form action="indexSearch.do" class="search_form fr" method="post">
				...
				<input type="text" class="search_text" value="" name="articleTitle"/> <input
					type="image" src="img/search_btn.gif" class="search_btn"
					alt="查找" title="search" />
					...
					<div id="" class="column_content fl">
						...
						<ul>
							...
							<c:forEach items="${articleList}" var="article" >
								<li>
								<a href="<%= path%>/four/${article.getArticleId()}.do">${article.getArticleTitle() }</a>
								<span class="fr ar">${article.getUploadTime()}</span>
								</li>
						...
						<div id="page">
							...
							<c:choose>
								<c:when test="${page.pageNum - 1 > 0}">
									<a
										class="page" href="<%= path %>/indexSearch.do?pageNum=${page.pageNum - 1}&articleTitle=${sessionScope.articleTitle}">[上一页]</a>
								</c:when>
								<c:when test="${page.pageNum - 1 <= 0}">
									<a class="page" href="/LTXProject/indexSearch.do?pageNum=1&articleTitle=${sessionScope.articleTitle}">[上一页]</a>
								</c:when>
							</c:choose>
							<c:choose>
								<c:when test="${page.pageNum + 1 < page.totalPageCount}">
									<a
										class="page" href="/LTXProject/indexSearch.do?pageNum=${page.pageNum + 1}&articleTitle=${sessionScope.articleTitle}">[下一页]</a>
								</c:when>
								<c:when test="${page.pageNum + 1 >= page.totalPageCount}">
									<a
										class="page" href="/LTXProject/indexSearch.do?pageNum=${page.totalPageCount}&articleTitle=${sessionScope.articleTitle}&articleType=${articleType} ">[下一页]</a>
								</c:when>
							</c:choose>
